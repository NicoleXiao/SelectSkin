...
namespace FancyScrollView
{
    public abstract class FancyScrollView<TData, TContext> : MonoBehaviour where TContext : class
    {
        ...
        protected Action<TData> OnCenterEvent;
        ...
        FancyScrollViewCell<TData, TContext> CreateCell()
        {
            ...
            cell.GetComponent<FancyScrollViewCell<TData>>().SetOnCenterEvent(OnCenterEvent);
            cell.SetContext(Context);
            cell.SetVisible(false);
            var rect = cellContainer.transform as RectTransform;
            cell.SetMoveWidth(rect.sizeDelta.x);
        ...
        public void OnCenter(int index)
        {
            for (int i = 0; i < cells.Count; i++)
            {
                ...
                else
                {
                    if (i > index)
                    {
                        var total = cells.Count + index;
                        cell.transform.SetSiblingIndex(total - i);
                    }
                    else if (i < index)
                    {
